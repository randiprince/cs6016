            ...
            using (MySqlConnection conn = new MySqlConnection(connection))
            {
                try
                {
                    ...
                    string query = "SELECT event.Name AS EventName, event.Site AS Site, event.Date AS Date,\n" +
                        ...
                        "WHERE TRUE"; // added where true in case parameters not null and need to add additional conditions/filters

                    if (white != null) // if user passed value for white, we want our query to be where whites player name equals this
                    {
                        query += " AND playerWhite.Name = @PlayerWhiteName"; //set PlayerWhiteName as white below 
                    }
                    if (black != null)
                    {
                    ...
                    if (useDate)
                    {
                        query += " AND Date >= @StartDate AND Date <= @EndDate"; //gives use events within a range
                    }
